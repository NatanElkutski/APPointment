...
public class ManagerSelectHoursFragment extends Fragment {
    ...
    CollectionReference barberWorkingDaysColRef;


    AlertDialog dialog;

    public ManagerSelectHoursFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View itemView = inflater.inflate(R.layout.fragment_manager_select_hours, container, false);
        dialog = new SpotsDialog.Builder().setContext(getActivity()).setCancelable(false).build();
        dialog.show();
        materialSpinner = itemView.findViewById(R.id.Manager_selectHours_materialSpinner);
        ...
        confirmbtn = itemView.findViewById(R.id.Manager_selectHours_confirm_button);
        removeDaybtn = itemView.findViewById(R.id.Manager_selectHours_close_selected_days);
        daysLinearLayout = itemView.findViewById(R.id.Manager_selectHours_days_main_ll);
        confirmbtn.setEnabled(false);
        removeDaybtn.setEnabled(false);
        loadSalonBarbers();
        return itemView;
    }
    ...
    public void barbersLoadSuccess() {
        ...
        materialSpinner.setOnItemSelectedListener(new MaterialSpinner.OnItemSelectedListener() {
            ...
            public void onItemSelected(MaterialSpinner view, int position, long id, Object item) {
                if (position > 0) {
                    if (daysLinearLayout.getVisibility() == View.VISIBLE)
                        daysLinearLayout.setVisibility(View.INVISIBLE);
    ...
    private void showBarberWorkingHours() {
        ...
        barberWorkingDaysColRef = db.collection("AllSalon")
                .document(Common.currentUser.getSalonCity())
                .collection("Branch")
                .document(Common.currentUser.getSalonId())
                .collection("Barbers")
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        for (int i = 0; i < 7; i++) {
            idArray = getResources().getIdentifier("Manager_selectHours_check" + (i + 1), "id", getActivity().getPackageName());
            c[i] = (CheckBox) view.findViewById(idArray);
        }

        for (int i = 0; i < 7; i++) {
            for (int j = 0; j < 2; j++) {
                idArray = getResources().getIdentifier("Manager_selectHours_day" + (i + 1) + "" + (j + 1), "id", getActivity().getPackageName());
                dayOfWeek[i][j] = (TextView) view.findViewById(idArray);
            }
    ...
    private void insertHoursToDB(String[] s1, String[] s2, String dayName) {
        ...
        if (temp2 > temp1) {
            ...
            DocumentReference dayReference = db.collection("AllSalon").document(Common.currentUser.getSalonCity()).collection("Branch").document(Common.currentUser.getSalonId())
                    .collection("Barbers").document(choosenBarberId).collection("WorkingDays").document(dayName);
            int dayNum = 0;
            switch (dayName) {
                case "Sunday":
                    dayNum = 1;
                            ...
                            try {
                                ...
                                if (date.get(Calendar.DAY_OF_WEEK) == finalDayNum) {
                                    ...
                                    CollectionReference baberBookingRef = db.collection("AllSalon").document(Common.currentUser.getSalonCity()).collection("Branch").document(Common.currentUser.getSalonId())
                                            .collection("Barbers").document(choosenBarberId).collection("Bookings").document("FutureBookings").collection(calendarStringDates.get(i));
                                                    ...
                                                    for (DocumentSnapshot hourSnapshot : dateSnapshot) {
                                                        ...
                                                        if (userHour < temp1) {
                                                            DocumentReference baberBookingRef = db.collection("AllSalon").document(Common.currentUser.getSalonCity()).collection("Branch").document(Common.currentUser.getSalonId())
                                                                    .collection("Barbers").document(choosenBarberId).collection("Bookings").document("FutureBookings").collection(barDate).document(docId);
    ...
    private void removeDayFromDB(String dayName) {
